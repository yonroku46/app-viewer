    ...
    <section className='signup fullsize'>
      {isComplete ? 
        ...
        <form className='signup-form' onSubmit={onSubmitHandler}>
          ...
          <div className='name'>
            <label>ユーザー名</label>
            <input type='text' id='name' autoComplete='username' className={name.length > 0 ? 'entered' : ''} placeholder='ユーザー名を入力' value={name} onChange={(e) => {setName(e.target.value); errReset();}}/>
          </div>
          <div className='email'>
            <label>メールアドレス</label>
